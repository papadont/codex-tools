...
- Current production revision: `codex-memo-remote-mcp-00008-k57`
...
承認前にはCloud Run、Firestore、Secret Managerを変更しない。
...
- target revisionの`npm test`が全件成功
- candidateのCloud Run設定が現行と一致
- TTL policyが有効
- candidateとproduction URLのread-only smokeが成功
- controlled write canaryを承認した場合、その作成・retry・attachment・cleanupが成功
- rollback先revisionと実行コマンドを手元に保持
